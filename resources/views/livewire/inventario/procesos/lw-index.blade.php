    ...
    <x-header-table>
                ...
                <ul class=" dropdown-menu
                    ...
                    <li>
                        <div class="form-check ml-2">
                            <input wire:model.defer='type' value='descripcion'
                                class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600  mt-1 align-top  mr-2"
                                type="radio" name="flexRadioDefault" id="flexRadioDefault1">
                            <label class="form-check-label inline-block text-gray-800" for="flexRadioDefault1">
                                Descripción
        ...
        <div class="container-fluid flex">
            <div class="m-1 flex flex-row text-right">
                <a type="button" href="{{ route('procesos.productos', $producto->id) }}"
    ...
    <x-table>
        <table class="min-w-full">
            @if ($procesos->count())
                ...
                <tbody>
                    @foreach ($procesos as $pros)
                        <tr class="bg-white border-b">
                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                {{ $pros->proceso->nombre }}</td>
                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                {{ substr($pros->proceso->descripcion, 0, 60) }}
                            </td>
                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                @if ($pros->proceso->maquinaria)
                                    {{ $pros->proceso->maquinaria->nombre }}
                                @else
                                    Ninguna
                                @endif
                            </td>
                            <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                <div class="inline-flex" role="group">
                                    <button type="button" wire:click='delete({{ $pros->id }} )'
        ...
        <x-pagination :modelo='$procesos'> </x-pagination>
    </x-table>
</div>
